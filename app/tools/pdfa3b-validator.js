// tools/pdfa3b-validator-strict.js
//#!/usr/bin/env node
const fs = require("fs");
...
/**
 * validatePDFA3bStrict(pdfBuffer)
...
function checkOutputIntents(pdf) {
  ...
  // must be array-like: size() and get()
  ...
  for (let i = 0; i < oiArr.size(); i++) {
    ...
    const Sraw = oi.get(PDFName.of("S"));
    const S = resolveName(Sraw, ctx);
    ...
    // try to validate ICC: attempt to read raw then inflate if necessary
    const iccErrors = validateICCStream(destStream);
    errors.push(...iccErrors);
    if (iccErrors.length === 0) hasValidICC = true;

    if (!S) warnings.push("OutputIntent /S missing.");
    else if (S !== "GTS_PDFA1") warnings.push(`OutputIntent /S is "${S}" (expected "GTS_PDFA1").`);
...
function validateICCStream(pdfStream) {
  // pdfStream: PDFStream-like with getContents()
  const errors = [];
  let bytes;
  try {
    bytes = pdfStream.getContents();
  } catch (e) {
    errors.push("Could not read ICC stream contents.");
    return errors;
  }
  if (!bytes || bytes.length === 0) {
    errors.push("ICC stream empty.");
    return errors;
  }

  let buf = Buffer.from(bytes);

  // If there's a Flate/Deflate wrapper, try inflating (best-effort).
  // Some streams are stored compressed in the PDF; we need actual ICC file bytes.
  try {
    const inflated = zlib.inflateSync(buf);
    if (inflated && inflated.length > 0) buf = inflated;
  } catch (e) {
    // not inflated / not compressed — continue with original buf
  }

  // check size and 'acsp' signature at byte offset 36..39
  if (buf.length < 40) {
    errors.push("ICC profile too small.");
    return errors;
  }
  const sig = buf.slice(36, 40).toString("ascii");
  if (sig !== "acsp") errors.push('ICC profile missing "acsp" signature.');
  const cs = buf.slice(16, 20).toString("ascii").replace(/\0/g, "");
  if (!["RGB ", "CMYK", "GRAY"].includes(cs)) {
    // some ICCs have slightly different codes; flag but not fatal
    errors.push(`ICC profile color space is "${cs}", expected RGB/CMYK/GRAY.`);
  }
  return errors;
}

// XMP: ensure it's present, a stream and contains rdf and pdfaid info
async function checkXMP(pdf) {
  ...
  let bytes = mdStream.getContents();
  if (!bytes || bytes.length === 0) {
    errors.push("XMP metadata stream empty.");
    return { errors, warnings, report };
  }

  // try inflate if content looks compressed (best-effort)
  let buf = Buffer.from(bytes);
  try {
    const inflated = zlib.inflateSync(buf);
    if (inflated && inflated.length > 0) buf = inflated;
  ...
  // parse xml lightly (fast-xml-parser) to ensure it's well-formed
  try {
    const parser = new XMLParser({ ignoreAttributes: false, parseTagValue: false });
    parser.parse(xml); // will throw if invalid
    // look for pdfaid:part and pdfaid:conformance in raw xml as well
    const mPart = xml.match(/pdfaid:part.*?>(\s*\d+\s*)</i);
    const mConf = xml.match(/pdfaid:conformance.*?>(\s*\w+\s*)</i);
    if (mPart) report.part = Number(mPart[1].trim());
    else errors.push("XMP missing pdfaid:part=3.");
    if (mConf) report.conformance = mConf[1].trim();
    else errors.push("XMP missing pdfaid:conformance=B.");
  } catch (e) {
    errors.push("XMP XML parsing error.");
  }
  ...
  for (const [, obj] of ctx.enumerateIndirectObjects()) {
    ...
    for (const cand of candidates) {
      ...
      if (/JavaScript/i.test(S)) {
        errors.push("JavaScript action found.");
        report.jsActions.push(S);
      }
      if (/Launch/i.test(S)) {
        errors.push("Launch action found.");
        report.launchActions.push(S);
      }
      if (/RichMedia|Movie|Sound/i.test(S)) {
        errors.push(`${S} action found.`);
        report.richMedia.push(S);
      }
    }
  }
...
function checkColorSpaces(pdf) {
  ...
  for (const page of pages) {
    ...
    const handleCS = (val) => {
      const csName = val && typeof val.value === "function" ? val.value() : val && val.value ? val.value : null;
    ...
    if (colorSpaces && typeof colorSpaces.get === "function") {
      for (const key of colorSpaces.keys()) handleCS(deref(colorSpaces.get(key), ctx));
    } else if (colorSpaces && colorSpaces.value) handleCS(colorSpaces);
    else if (colorSpaces && typeof colorSpaces.size === "function") {
...
function deref(obj, ctx) {
  try {
    return obj && obj.lookup ? obj.lookup(ctx) : obj;
  } catch (e) {
    return null;
  }
}

function resolveName(obj, ctx) {
  if (!obj) return null;
  try {
    // pdf-lib PDFName often exposes value() method
...
function resolveObjectToString(obj, ctx) {
  if (!obj) return null;
  try {
    if (typeof obj.value === "function") return obj.value();
    if (typeof obj.value === "string") return obj.value;
