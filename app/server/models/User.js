...
if (!ENCRYPTION_KEY || ENCRYPTION_KEY.length !== 32) {
  throw new Error(
    `ENCRYPTION_KEY must be set in the .env file and must be 32 characters long. Current length: ${ENCRYPTION_KEY?.length}`
  );
}
...
userSchema.pre("save", async function (next) {
  ...
  if (this.isModified("wooConsumerSecret") && this.wooConsumerSecret)
    this.wooConsumerSecret = encrypt(this.wooConsumerSecret);
  next();
});
