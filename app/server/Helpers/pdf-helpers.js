...
<?xpacket end="w"?>`;
  return { xmpString, documentId, instanceId };
}
...
async function finalizePdf(originalPdfBuffer, invoiceData) {
  ...
  // 5. Add XMP metadata
  const { xmpString, documentId, instanceId } = generatePdfA3bXmp(invoiceData);
  ...
  // Explicitly set the PDF trailer ID to match the XMP DocumentID and InstanceID
  const pdfDocumentId = PDFHexString.fromText(documentId.replace('uuid:', ''));
  const pdfInstanceId = PDFHexString.fromText(instanceId.replace('uuid:', ''));
  pdfDoc.context.trailer.set(PDFName.of('ID'), pdfDoc.context.obj([
    pdfDocumentId,
    pdfInstanceId
  ]));
...
async function convertToPdfA3b(pdfBuffer, invoiceData) {
  ...
  const { xmpString: xmp } = generatePdfA3bXmp(invoiceData); // Use xmpString from the returned object
  pdfDoc.catalog.set(
    PDFName.of('Metadata'),
